...
@section('contents')
    ...
    <section class="container mx-auto mt-10">
        <h2 class="text-4xl text-center font-black my-10">Stories</h2>
        @if ($posts->count())
            <div class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach ($posts as $post)
                    <div>
                        <a href="{{ route('posts.show', $post) }}">
                            <img src="{{ asset('uploads') . "/" . $post->imagen }}" alt="{{ $post->titulo }} uploaded picture" > 
                        </a>
                    </div>
                @endforeach
            </div>
        ...
        @else
            <p class="text-gray-600 uppercase text-sm text-center font-bold">No stories yet!</p>
        @endif
